...
Level 1 edits *this* codebase. Level 2 treats the **game itself as a prompt**: instead of cloning the repo, you describe the game to an **AI builder** (an AI coding agent like Claude Code or Cursor, or an app builder like Replit Agent, Bolt, v0, or Lovable) and have it generate the whole thing for you — then refine it in conversation. This is, in fact, how Pill Match was built, and it's the fastest way to spin up your *own* mechanics-as-message game or to rebuild on a different stack.

**→ See [REFERENCE_PROMPT.md](REFERENCE_PROMPT.md)** for the full, self-contained prompt. It includes:

- a complete **"Required mechanics (must all be present)"** checklist — including the smooth **gravity / refill / cascade animations** that builders most often skip;
- a **"Confirmation before you build"** step that makes the AI present its plan — theme, tokens, rules, mechanics, *and* the Level 1 forking options — and **wait for your sign-off before writing code**;
- stack-substitution guidance, a done-definition, a per-builder notes table, and after-generation checks.

For a **creative fork**, change the theme, tokens, rules, and prescription content during that confirmation step — the same Match-3 skeleton will carry a completely different meaning, which is the whole idea.
